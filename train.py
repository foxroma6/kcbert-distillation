...
MODEL_CLASSES = {
    ...
    'distilkcbert': (DistilBertConfig, DistilBertForMaskedLM, AutoTokenizer),
    'roberta': (RobertaConfig, RobertaForMaskedLM, RobertaTokenizer),
    'kcbert': (BertConfig, BertForMaskedLM, AutoTokenizer),
...
def sanity_checks(args):
    ...
    if args.mlm:
        ...
        assert (args.student_type in ['roberta', 'distilkcbert', 'distilbert']) and (args.teacher_type in ['roberta', 'kcbert', 'bert'])
    else:
        assert (args.student_type in ['gpt2']) and (args.teacher_type in ['gpt2'])

    assert args.teacher_type == args.student_type or (args.student_type == 'distilbert' and args.teacher_type == 'bert') \
        or (args.student_type == 'distilkcbert' and args.teacher_type == 'kcbert')
...
def main():
    ...
    parser.add_argument("--student_type", type=str, choices=["distilbert", "roberta", "gpt2", "distilkcbert"], required=True,
                        help="The student type (DistilKcbert, DistilBERT, RoBERTa).")
    ...
    parser.add_argument("--teacher_type", choices=["bert", "roberta", "gpt2", "kcbert"], required=True,
                        help="Teacher type (Kcbert, BERT, RoBERTa).")
    ...
    if args.is_master:
        if os.path.exists(args.dump_path):
            if not args.force:
                raise ValueError(f'Serialization dir {args.dump_path} already exists, but you have not precised whether to overwrite it'
    ...
    ### TOKENIZER ###
    if teacher_tokenizer_class == AutoTokenizer:
